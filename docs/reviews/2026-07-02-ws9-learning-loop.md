...
| 6 | First e2e run reported exit 0 while 14/18 tests failed, and the details were lost | piped through `\| tail -20` — pipeline exit code + truncated log | re-ran with full log to a file; diagnosed properly | never pipe a test runner's output when its exit code and failure details matter |
| 7 | e2e suite had NEVER actually run: invalid teudat-zehut fixture silently blocked the wizard; tests targeted a `<select>` replaced months earlier | suite added "one-shot" with no browser in the container and no CI e2e step | fixed fixture (valid check digit), rewrote tests against the real UI, `PLAYWRIGHT_CHROMIUM_PATH` for containers — 18/18 green | a test that has never failed has also never run: wire e2e into CI (Proposal P1) |
