...
## 🟢 מילסטון 2026-06-10 — דמו → פיילוט SaaS חי (סטטוס נכון ל-13.6)

עברנו מ-דמו-localStorage ל-**פלטפורמת SaaS חיה בפרודקשן** (`countmedemo-eight.vercel.app`). **10 PRs מוזגו ל-main (#15–#20), אפס שבירות.** הקוד אומת ב-main.

### סטטוס מול הלו"ז השבועי (היום שבת 13.6 = סוף שבוע-1)
| שבוע | יעד-מוצר בתוכנית | מצב |
|---|---|---|
| **1** (7–11.6) | Phase 0-1: Supabase+auth+schema + ספייק | ✅ **DONE** · self-test מייסדים ⏳ (ידני) |
| **2** (14–18.6) | Phase 2-4: DB swap, auth, proxy | ✅ **DONE — נמשך קדימה לשבוע-1** |
| **3** (21–25.6) | Phase 5-6: דשבורד + seed | ✅ **DONE** (seed=self-serve) — נמשך קדימה |
| בונוס | — | דיוק-2025 · design-fidelity · chat rail · ניקוי-דאטה · GTM/PMF brief |

**שורה תחתונה: ציר המוצר ~2 שבועות לפני.** הצוואר עכשיו = הצירים הלא-טכניים (משפטי→סייבר→תמחור, שיווק) + 2 שערים ידניים — **לא הבנייה.** יעד השקת-פיילוט (שבוע 5, ~5-7.7) בר-השגה, מותנה במשפטי + הידני.

### ⚠️ תיקוני-דיוק (מול סיכום Gemini — לא לתכלל)
- **Gating כבוי** — בנוי+דגל `AUTH_GATING_ENABLED`, לא מופעל. "נעול/מוכן ל-EY" מותנה: הדלקה (יוני) + self-test + אישור יעל.
- **כיול 2025 ~90%** — NEEDS-ROY פתוח (תקרות פנסיה 2025, פיצול מס-בריאות, מילואים מול תיקון 283).

### קוד שנשלח (אומת ב-main)
`src/lib/supabase/*`(5) · `src/lib/data/*`(3) · `src/app/{login,auth}` · `middleware.ts`+`proxy.ts` · `src/components/dashboard/*`(10) · `chat-nav-rail` · `supabase/migrations/*` · קבועי-2025 ב-`calculators/types.ts` · `docs/{audit,gtm,spikes}/*`.

### צוות (מתוקן) ונושאי-פגישות
**ליבה:** תומי (שיווק) · רוי (פיננסי/רגולציה) · יוני (CTO). **חיצוניים:** יעל (פרטיות) · מאיה (רו"ח/EY) · **שי (סייבר = אבא של תום-לב)** · פרל-כהן (תאגידי).
- **יעל** (השער לחיצוני): הודעת-פרטיות+הסכמה (L1/L2), רישום מאגר (R1/תיקון 13 — כעת PII אמיתי), DPA (L6), קופי "מחליף רו"ח" מול companion/דיסקליימר (§3). "מה עוד?"
- **מאיה:** גבול רו"ח↔AI (#1) כעת שהמוצר מדויק+עובדתי; מודל אחריות; מוכנות דמו-EY.
- **שי:** RLS חי מעל דאטה אמיתי → Pen-Test; ניהול מפתחות (service-role+ריבוי-חשבונות); תיקון-הדליפה; הצפנה.
- **פרל-כהן:** הסכם מייסדים (L3); ויתור/IP ליה (L4); בע"מ אחרי פיילוט.
- **ליבה:** רוי→NEEDS-ROY+software-approval(R2); תומי→GTM/PMF brief+landing נקי; יוני→2 השערים+design-review.
