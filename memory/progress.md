...
> החדש למעלה. מקור-אמת ההיסטורי למצב המשימות: `docs/archive/yoni-tasks-27032026.md` (ארכוב 2026-08-11).

## סבב 11/08/2026 — עדכונים רוחביים: מנוע הוצאות 2026 + מסקוט + onboarding + expenses + pruning

יוני העביר חבילה גדולה: דאטהסט הוצאות מוכרות 2026 (xlsx קנוני, 62 כללים/113 מקצועות/617 הוצאות),
תמונת מסקוט-שקל שמחליפה את "איתן", ארטיפקטים מלאים (קוד+מפרט) להעלאת-הוצאות ו-onboarding, מפה
רגולטורית. ביקש שימוש ב-dynamic workflows/agents מקבילים ופרונינג בסיום. פורט מלא בתדריך העדכני
ב-[[STATUS]] (§ "11/08"); כאן רק ציר-הזמן בפועל.

**זרימת העבודה (Fable, ענף `claude/system-updates-optimization-ah8z45`, 7 קומיטים):**
1. תכנון-מלא (plan mode) → קריאת כל שלושת הקבצים שהועלו (xlsx/pdf הוצאות, pdf רגולטורי) + חילוץ
   ידני מלא שלהם (הארטיפקטים החסומים מה-URL — נפתר בהמשך בהדבקה ישירה של יוני).
2. יוני אישר את התוכנית + הדביק את שלושת הארטיפקטים במלואם (כולל קוד HTML/React גולמי) — עיבוד שני
   סבב, זיהוי 2 קונפליקטים אמיתיים מול CLAUDE.md הנעול (פלטת-צבעים שונה, מסכי-auth כפולים+"חברה
   בע״מ" בתוך ה-onboarding-artifact) — הוכרעו במפורש ותועדו כפערים, לא הוכרעו בשקט.
3. **שלב A (ישיר + Workflow מקבילי):** בניתי בעצמי את `src/lib/expense-engine/` (גנרטור xlsx→TS,
   טיפוסים, API, golden test) — כולל שחזור-מתועד של 3 שורות חסרות במקור (VEH-01/P001/DEP-01,
   מאומתות-צולבות) ואי-שחזור מכוון של 2 (EB-001/EX-0001, אין מקור-אימות). Workflow מקבילי (2 agents)
   חיבר את הדאטה ל-`/business-expenses` + כלי-צ'אט `get_expense_rule`.
4. **שלבים B/C1/C2 (3 background agents מקבילים):** מסקוט (`character.ts` + סוויפ 20 קבצים) ·
   `/onboarding` (מלא, ONB-1..ONB-11) · `/expenses` (זרימת-הוצאות בטא מלאה). כל אחד אימת tsc+vitest
   עצמאית לפני דיווח; אני אימתתי מחדש את השילוב המלא (tsc+vitest+build+Playwright אמיתי) לפני commit.
5. **שלב D — pruning:** ראה [[STATUS]] לפירוט. תג `pre-prune-2026-08` (push נכשל, 403 הרשאות) ·
   מחיקות (form-screenshots, Brand-Kit HTML) · ארכוב ל-`docs/archive/`+`memory/archive/` · CLAUDE.md
   372→160 שורות (עדכון-מלא, לא cherry-pick עיוור מהענף התקוע) · סקילים 18→8 · puppeteer→playwright
   (אומת שימוש אמיתי לפני שינוי; smoke test 11/11 אחרי).

**לקח מתודולוגי:** בדיקת-דפדפן אמיתית (Playwright, לא רק `--screenshot` חד-פעמי) תפסה false-positive
משלי — screenshot ראשון של `/onboarding` "הראה" מסך ריק, התברר שזו רק אנימציית-Reveal (framer-motion
`useInView`) שלא הספיקה להשלים לפני שהצילום נלקח; המתנה של 1.2 שנ' חשפה תוכן תקין לגמרי. תיעוד למי
שיריץ בדיקות דומות בעתיד — screenshot חד-פעמי לא שווה-ערך לחוויית משתמש אמיתית.

**מה נשאר פתוח:** ראה רשימת-הפערים המלאה ב-[[STATUS]] § "11/08" — הכי דחוף: שער `/file`/`/demo`
על `journey.filingDetailsCompleted` (משתמש-חדש יכול לראות "מחשבון" עם נתוני-0 בלי אזהרה).
