...
> עודכן: 2026-08-03 (Sonnet) · ענף: `claude/planning-clean-beta-q3k1vl` · החלטות: [[decisions]]

## 🎯 03/08 — שחזור-מצב אחרי פער-בזיכרון + אודיט-בטא מקיף (32 סוכנים, 28/28 ממצאים מאומתים)

> סשן מחקר/תדרוך טהור — **אפס שינוי קוד, אפס push**. יוני פתח סשן חדש וביקש תדרוך על מצב הבטא +
> רשימת משימות ליוני/תומי/רועי. שני ממצאים קריטיים עלו תוך כדי:

- **פער-זיכרון של 10 ימים אותר:** הכותרת הזו (19/07) הייתה הרשומה האחרונה כאן למרות שעבודה
  אמיתית נמשכה עד 26/07 — על ענף מרוחק **`claude/system-beta-preparation-oiyzpy` שאף פעם לא
  קיבל PR ולא מוזג** (אותר רק דרך `git fetch` יזום, לא הופיע ב-`git log` המקומי). הענף מכיל:
  קיצור CLAUDE.md (372→142 שורות), ניקוי 6 לינקי-ניווט ישנים ל-1301/דמו, `memory/regulatory-status.md`
  חדש (מעקב רגולטורי — 12 פריטים, רובם 🔴/🟡), Routine יומי אוטומטי חדש (`trig_015E885nv2xuKdhxTd9am8tV`,
  ביקורת קוד כל יום 03:00 UTC), ותיקון ל-`docs/launch/oauth-branding.md` — שחשף באג חי **שעדיין
  לא תוקן**: Redirect URLs ב-Supabase (hbsgz) חסרים את הדומיין הקנוני האמיתי `countmedemo-eight.vercel.app`.
  אומת מול Vercel MCP: **פרודקשן חי היום על `2fe566d` (PR #31) — בלי אף אחד מתיקוני ה-26/07**.
- **אודיט-בטא מקיף (Workflow, 32 סוכנים, 4 מימדים — אבטחה/UX/שלמות-נתונים/תפעול — עם אימות-אדוורסרי
  לכל ממצא):** 28 ממצאים, 28/28 אומתו נגד הקוד בפועל. **התיקון הכי חשוב — סותר הנחה קודמת מהיום
  הזה עצמו:** תיקון "אובדן-נתונים ממכשיר-שני" מ-PR #31 (`useRequiredPersona`) הוחל על 5 בלבד
  מתוך ~14 דפים שקוראים persona — `business-expenses`, `coach`, `deadlines`, `alerts`,
  `dashboard/pro`, `setup/assets`, `file/*` ועוד עדיין עם ה-guard הישן, עדיין חשופים לאותה
  מחלקת-באג. **בלוקר נוסף:** הפקת-מסמכים אינה אטומית (`persistPersona`/`upsertPersona` fire-and-forget)
  — יכולה לאבד מסמך שהופק או לשכפל מספר-מס חוקי בין שני מכשירים/כרטיסיות. `/api/doc-link` מאפשר
  זיוף מסמך ממותג-countme בלי בדיקת בעלות בשרת. Next.js נעול על 16.2.10 (7 חולשות, 5 גבוהות,
  תיקון זמין ל-16.2.12). ה-runbook עצמו (`docs/launch/oauth-branding.md`) עדיין מפנה בפועל
  לפרויקט Supabase הנטוש. CI לא רץ על ענפים בלי PR פתוח — מסביר למה ענף ה-26/07 מעולם לא נבדק.
  אונבורדינג הקליל (רכיב #1 של תומי) עדיין לא נבנה בכלל. פירוט מלא + כל 28 הממצאים (severity/
  file/fix) נמסרו ליוני בצ'אט — לא שוכפלו כאן במלואם, רק הבלוקרים.
- **מסלול מומלץ שנמסר ליוני:** למזג/לפתוח PR לענף `claude/system-beta-preparation-oiyzpy` +
  לתקן את באג ה-Redirect URL + לסגור את בלוקר "התיקון החלקי" **לפני** הרצף הקיים ב-
  `docs/launch/beta-go-live-runbook.md` (env→gating→תקציב→ניקוי→QA→שחרור). ה-onboarding החסר
  נשאר החלטה פתוחה ליוני: לבנות לפני שחרור ראשונה, או לשחרר על `/setup` הקיים ולהוסיף אחר-כך.
- **⚠️ הטבלה "חסום — ממתין" למטה (מסבב 10/06) לא עודכנה עדיין עם הבלוקרים החדשים מלמעלה** — הסשן
  הבא שמבצע קוד צריך לאחד אותה עם הרשימה כאן לפני שהיא משמשת כמקור-אמת יחיד.
