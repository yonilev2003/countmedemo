...
- **CLAUDE.md rewrite + regulatory-status.md** מ-`claude/system-beta-preparation-oiyzpy` (26/07) —
  נשארו לא-ממוזגים בכוונה. שכתוב CLAUDE.md (372→142 שורות) לא נבדק מול המצב הנוכחי בזמן הסשן הזה;
  regulatory-status.md כנראה עדיין בטוח להוסיף (12 פריטים, מעקב רגולטורי בלבד) — לבדוק מול הריפו
  הנוכחי ולמזג בנפרד.
