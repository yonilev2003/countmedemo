...
> עודכן: 2026-07-03 (סשן-צד: אימות מס 2025/2026 מול האינטרנט) · התדריך התפעולי (גייטינג וכו') מ-02/07 עדיין בתוקף — ראו סעיף 🎯 למטה.

## 🔬 סשן-צד 03/07 — אימות קבועי-מס מול האינטרנט (ענף `claude/tax-rules-version-hvnzt3`, 2 commits, טרם PR)

**מה זה היה:** סשן בדיקות בלבד (לא פיתוח). כל קבוע 2025/2026 ב-`types.ts` אומת מול האינטרנט
ב-workflow אדוורסרי (חוקר+מפריך לכל נושא, ~640 חיפושים סה"כ). **מספרי הדמו של דנה לא השתנו.**

**תוקן בקוד (commits `08395be`+`42ddd18`, push בוצע, build+101 tests+tsc ירוקים):**
- ב"ל עצמאי: שיעורים משולבים 7.70%/18.00% לשתי השנים (היו 7.12%/17.83% — ערכי טרום-2025; תיקון 252+69). קבועים רדומים.
- סף טופס 6111: **254,237** (=300,000 כולל מע"מ ÷1.18) ל-2025+2026; 256,410 נשאר רק ל-2024 (עידן 17%).
- תקרת הפקדה מוטבת קרן: **20,520** ל-2024/2025 (סבירות ~80% בלבד!), 20,566 רק מ-2026.
- קבוע חדש `surtaxCapitalIncomeAdditionalRate` (+2% הוני מ-2025, §121ב/תיקון 276) — **מוגדר אך לא נצרך באף מחשבון.**
- בלוק golden-tests חדש "2025 vs 2026 clean year separation" — הפרדת השנים נאכפת ב-CI.
- הערות: עוסק זעיר תיקון 257→**265** · מדרגת 20-יום מילואים 2027→**2028** (כולל טקסט user-facing ב-index.ts:494) · 5 סתירות-הערות נוספות.

**נחקר ואומת — בלי שינוי קוד (ממצאי מחקר 03/07, לטיפול בסשנים הבאים):**
1. **שדה 032 (ריבית/דיבידנד) נאסף אך לא ממוסה בכלל** ב-estimateTaxLiability. הדין (אומת ~85-90%):
   מס בשיעור קבוע §125ב/§125ג (ריבית 15%/25%, דיבידנד 25%/30% למהותי) — **אסור לערבב לתוך המדרגות**;
   נדרש מחשבון flat-rate נפרד (מבנה "חלק ה" של 1301). כרגע לא מזיק (דנה=0) אבל פער אמיתי.
2. **מס יסף** — אומת (~92%): חל לפי גובה הכנסה בלבד, אין פטור לעצמאים; ההוני נערם *מעל* יגיעה אישית.
   לא רלוונטי לקוהורט הפיילוט (150-400K). הקוד תקין.
3. **תגמולי מילואים ב"ל (לא מס!):** תוספת ה-**25% לעצמאי אומתה כאמיתית** (~92%, §272: הכנסה ממוצעת×1.25÷90) —
   אבל מותנית ב(א) עמידה בהגדרת "עובד עצמאי" של ב"ל (מבחן שעות/הכנסה — ≥20 ש"ש או הכנסה ≥50% שכר ממוצע
   או ≥12 ש"ש+15%) שהיא **שונה לגמרי מ-osekType** שאנחנו אוספים, ו(ב) תשלום דמי ביטוח סדיר. מי שלא עומד
   בהגדרה מקבל לפי השכר הממוצע במשק בלבד. **הסכימה שלנו לא יכולה להסיק זכאות (95%)** — צריך שדות חדשים.
4. **תוספת 40%:** קיימת אך היא gross-up ימי-מנוחה מותנה 60+ ימים מצטברים/30 רצופים, משולמת רק **בסוף הצו**
   (לא "מיידית"). לגבי צו 8 מול רגיל — עדויות סותרות בין החוקר למאמת (~70-80%); הכלל הבטוח: לעצמאים יש
   סיכון עיכוב+התחשבנות-בדיעבד (clawback, דוח מבקר 11/2025) → הודעת תזרים ב-UI, לא חישוב.
5. **יגיעה אישית:** הכנסת דנה 100% יגיעה אישית ✓. מקרי קצה (חברות ארנק, עסק עתיר-הון) לא רלוונטיים לפרסונה.

**הפרכת פברוק Gemini (03/07):** בשיחת Gemini מקבילה נטען שנוספו `checkKnowledgeGap`/"מדד ביטחון"/"מדד
מורכבות" לקוד ושנפתח PR — **שקר; נבדק בגרפ, אפס שורות כאלה.** רק שני ה-commits לעיל אמיתיים. לקח: פלט
מודל ללא גישת-כלים לריפו אינו עדות למצב הקוד.
