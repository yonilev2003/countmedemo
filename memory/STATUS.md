...
**נשאר פתוח לסשן הבא (בסדר עדיפות):**
- **`AUTH_GATING_ENABLED` עדיין כבוי** (חוסם הכי ותיק בפרויקט — ראה [[decisions]] לרשימה המלאה של
  צעדים ידניים ליוני).
- **Redirect URL חסר ב-Supabase (hbsgz):** הדומיין הקנוני האמיתי `countmedemo-eight.vercel.app` לא
  ברשימת ה-Redirect URLs — אותר באודיט 03/08, **עדיין לא תוקן** (זו פעולת-קונסולה, לא קוד).
- **הפקת-מסמכים לא-אטומית** (`persistPersona`/`upsertPersona` fire-and-forget) — יכולה לאבד מסמך או
  לשכפל מספר-מס חוקי בין שני מכשירים/כרטיסיות בו-זמנית. לא טופל בסשן הזה (דורש עיצוב — transaction/lock
  ברמת ה-DB, לא תיקון קוד מקומי).
- **Tranzila webhook — נשאר stub במכוון.** ניסיתי לוודא את סכמת החתימה האמיתית של Tranzila (חיפוש
  ברשת) ולא הצלחתי לאשש אותה מספיק כדי לממש בביטחון — ניחוש סכמת-הצפנה לתשלומים גרוע יותר מ-stub
  מתועד. דורש: יוני מחבר terminal secret אמיתי + תיעוד Tranzila הרשמי, ואז מימוש HMAC מול הספק.
- **CI לא רץ על ענפים בלי PR פתוח** — זו הסיבה שהשרשור מ-26/07 "נעלם". לא תוקן (זה בהגדרות הריפו ב-
  GitHub, לא בקוד) — יוני: להחליט אם רוצים `push:` trigger גם ל-branches בלי PR, או משמעת-תהליך.
- **CLAUDE.md rewrite** מ-`claude/system-beta-preparation-oiyzpy` (26/07) — נשאר לא-ממוזג בכוונה.
  שכתוב 372→142 שורות לא נבדק מול המצב הנוכחי בזמן הסשן הזה — צריך עין לפני מיזוג.
  ✅ **`memory/regulatory-status.md` נמשך בסבב-לולאה של אותו סשן** (03/08 מאוחר) — קובץ עצמאי, ללא
  קוד, בלי קונפליקטים — ראה [[regulatory-status]] למעקב אחר מסמכים/חובות משפטיות (12 פריטים,
  10 מסומנים 🔴 לא-הוערך — חוסם-העל: אין סוקר משפטי חיצוני פעיל מאז 02/07).
