...
const FEATURE_HE: Record<Feature, string> = {
  ...
  deduction_finder: `מאתר ההוצאות וההטבות של ${CHARACTER.name}`,
  coach_unlimited: `שיחות ללא הגבלה עם ${CHARACTER.name}`,
      ...
      <div className="grid gap-5 sm:grid-cols-2 sm:gap-6">
        ...
        <Reveal>
          <section className="flex h-full flex-col rounded-3xl border border-line bg-paper p-7 shadow-brand">
            ...
            <Link href={ONBOARDING_ROUTE} className={btn("secondary", "md", "mt-7 w-full")}>
              התחילו חינם
            </Link>
          </section>
        </Reveal>
        ...
        <Reveal delay={0.08}>
          <section className="relative flex h-full flex-col rounded-3xl border-2 border-brand-deep bg-brand-navy p-7 text-white shadow-brand">
            ...
            <p className="mt-1 text-sm text-aqua/80">הדוח כולו, מקצה לקצה, עם {CHARACTER.name} ללא הגבלה.</p>
            <ul className="mt-6 flex flex-1 flex-col gap-3">
