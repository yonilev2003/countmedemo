    ...
    <div className="min-h-screen bg-cream flex flex-col">
      ...
      <header className="bg-white border-b border-stone-200">
        <div className="mx-auto flex max-w-screen-xl items-center justify-between px-6 py-4">
          <Link href="/file" className="flex items-center gap-2 text-sm text-stone-600 hover:text-brand-navy">
            ← בחירת מסלול
          </Link>
          ...
          <Link href="/demo" className="rounded-full border border-brand-navy/20 px-3 py-1 text-xs text-brand-navy hover:bg-info/20">
            טופס Gov.il ←
          </Link>
        </div>
      </header>
          ...
          <div className="flex gap-1 items-center">
            {FORM_MODULES.map((m, idx) => (
              <div key={m.id} className={[
                ...
                idx < moduleIndex ? "bg-success" : idx === moduleIndex ? "bg-brand-navy" : "bg-stone-200",
              ].join(" ")} />
            ))}
          </div>
        </div>
      ...
      {savedFlash && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 rounded-full bg-success text-white px-4 py-1.5 text-xs font-medium shadow-lg">
          ✓ נשמר
        </div>
      )}
      ...
      <main className="flex-1 mx-auto w-full max-w-screen-md px-6 py-8">
        ...
        {/* Schema-based fields with inline edit for personal fields */}
        ...
        {/* Bank — module 5 (editable, copyable) */}
        {showBankInfo && (
          <div className="space-y-2 mb-4">
        ...
        {/* Sync note */}
        <p className="mt-4 text-center text-[11px] text-stone-400 leading-relaxed">
        ...
        <div className="mt-8 flex items-center justify-between">
          {moduleIndex > 0 ? (
            <button onClick={goBack} className="rounded-xl border border-stone-300 bg-white px-5 py-2.5 text-sm font-medium text-stone-700 hover:bg-stone-50 transition-colors">
              ← חזרה
            </button>
          ) : <div />}

          {isLast ? (
            <Link href="/demo" className="rounded-xl bg-success px-6 py-2.5 text-sm font-bold text-white hover:bg-success/90 transition-colors shadow-sm">
              סיום — פתח/י את טופס Gov.il →
            </Link>
          ) : (
            <button onClick={goNext} className="rounded-xl bg-brand-navy px-6 py-2.5 text-sm font-bold text-white hover:bg-brand-navy/90 transition-colors shadow-sm">
              המשך →
            </button>
          )}
        </div>
...
function ReadOnlyRow({ label, value, hint }: { label: string; value: string; hint?: string }) {
  return (
    <div className="flex items-center gap-3 rounded-lg border border-stone-200 bg-stone-50 px-4 py-2.5">
      <span className="flex-1 text-sm text-stone-600">{label}</span>
      <span className="font-semibold text-stone-700 text-sm" title={hint}>
        {value}
      </span>
    </div>
  );
...
function InlineEdit({
  ...
  return (
    <button
      onClick={startEdit}
      className="group flex items-center gap-1.5 rounded px-2 py-0.5 hover:bg-info/30 transition-colors"
      title="לחץ/י לעריכה"
    >
