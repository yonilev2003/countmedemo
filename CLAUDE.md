...
- **Artifact-first UX for creation flows (Yoni, 2026-08-17 — mandatory):** every document/data-creation page (invoices, receipts, expenses, onboarding) must follow the approved design artifacts' staged experience and look, including colors (via the brand tokens, never raw hex): staged screens with tap-cards for choices, live "X מתוך Y שדות חובה מולאו" counter, all-errors-at-once banner, read-only computed values with a "חושב" pill, live preview cards. `/expenses/new` is the reference implementation. New creation flows must match it — don't ship a long single-page editor.
...
5. `FLAG(Roy)` burn-down in `lib/calculators/types.ts` — **superseded 2026-09-08: there is no more standing "approval from Roy" blocker.** memory/decisions.md's Evidence/Review/Release entry (08/09) retired that framing — every constant now carries a `confidenceTier` in `TAX_CONSTANT_META` (Evidence, in code) and is a reviewable card at `/admin/review` (Review, tracked in `review_decisions`); Yoni is the initial approver, an external CPA reviewer can join later without a schema change. The from-2024 child-credit-points table fix (06/09) already landed in code as `strongly_supported` — it is a normal review-queue item now, not a special "waiting on Roy" case. Genuinely still open: §45א life-insurance ceiling has no cap constant modeled at all (flagged `unverified` in the registry, not just a comment) — see `/admin/review` or `memory/risk-gap.md` §7.4 for the live list, not this paragraph.
