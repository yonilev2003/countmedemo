...
### 2b. Auth (Card style) — `CountMe Auth Card.html`
An **alternate** auth design (a lighter, card-on-light reskin of an existing prototype) — coexists with the glass `CountMe Auth.html`. Pick one as the production auth; this card style is the more recent direction. Mobile (two iPhone frames: הרשמה + התחברות) and Web (two cards side by side). Static HTML/CSS + a tiny vanilla JS for the password eye-toggle.
- **Layout:** centered logo (¢ + CountMe) → title → subtitle → white **card** (`--paper`, radius 24px, soft shadow) on a sand-50 page background. No background photo/glass.
- **Fields:** filled inputs (`--sand-50` bg, `--line` border, radius 14px, 54px tall), right-aligned RTL; **email** has an envelope icon, **password** has a lock icon + an **eye toggle** (vanilla JS flips input type) and shows masked dots. Focus state = teal border + aqua-soft glow.
- **Register:** "לפחות 6 תווים" helper under password. **Login:** "שכחתי סיסמה" link (teal).
- **Buttons:** primary = **navy pill** (56px, "הרשמה"/"התחברות"); then an **"או"** divider; then a **white outline Google pill** ("הרשמה/התחברות עם Google").
- **Footer:** register → "יש לך כבר חשבון? התחברות"; login → "אין לך חשבון עדיין? הרשמה". (No "מדריך הפעלה" link — removed.)
...
### 8. Marketing Landing Page — `CountMe Landing.html`
The public marketing site (responsive web — desktop + mobile browser; **not** an in-app native screen). Single scrolling page, RTL, no emoji in chrome. Sections top→bottom:
- **Sticky nav:** logo + links (פעולות מהירות, מילוי 1301, איתן, קהילה, מחירים) + "כניסה" (white outline pill) and "התחילו עכשיו" (navy pill). Links collapse below 900px — **needs a hamburger menu in production.**
- **Hero:** navy-teal radial+linear gradient; badge pill; two-line headline **"לשחרר את העצמאים" + beige "מהפחד"** (`h1 .b` block, beige); lede paragraph with a white-emphasized clause; single beige CTA "התחל/י עכשיו". Headline/text sizes use `clamp()`.
- **Shortcuts** (`#shortcuts`, sand bg): 4 cards (קבלה / חשבון עסקה / הצעת מחיר / לוח הבקרה) — icon tile, title, blurb, ⌘ key. 4-up → 2-up below 900px.
- **1301 + Eitan** (`#form1301`): two-column — a **טופס 1301** form mock (rows of code/label/value, one highlighted "refund found" row, footer total) beside a live **Eitan chat** ("איתן הנציג הדיגיטלי שלנו") with avatar, bot/user bubbles, an inline pill result, and a mic input. Stacks below 900px.
- **Social** (`#social`, navy bg): auto-scrolling **marquee** of Instagram/LinkedIn testimonial cards (CSS `scrollx` keyframe, edge mask, pause-on-hover). Cards are **built in JS** from a `posts[]` array (avatar initial, name, handle, platform icon, quote, body, hashtags) and duplicated for a seamless loop.
- **Final CTA:** dark gradient card, headline + "פתחו חשבון חינם" / "תכירו את איתן".
- **Footer:** brand + link columns (מוצר / חברה / משפטי) + copyright.
- **Production notes:** hero stat cards and a secondary hero CTA were intentionally removed — don't reintroduce. Testimonial names/quotes and any figures are **sample copy** — replace with real content. Wire nav/CTAs to the real auth/dashboard/chat routes.
...
- `CountMe Auth.html` — login / sign-up, frosted-glass style (mobile + web).
- `CountMe Auth Card.html` — login / sign-up, card-on-light style (mobile + web) — alternate, more recent.
...
- `CountMe Landing.html` — public marketing landing page (responsive web).
