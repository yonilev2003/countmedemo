...
1. **למזג את PR #13 (skills) ל-main** — ברגע שימוזג, *כל* סשן ענן יקבל את 13 הסקילים המרכזיים
   מהריפו אוטומטית, ואת הקטלוג המלא ב-`skills-lock.json`. עד אז הם קיימים רק בענף ה-skills.
   (פער הסקילים שהיה ה"קריטי ביותר" — ראו §4.א — **נפתר בסשן הזה**.)
2. **`Setup: בורר שנת מס`** — כרגע `income.year` קשיח ל-2024 (`src/app/setup/page.tsx`), כך
...
|---|---|---|---|
| **התקנת סקילים (skills-il): core+קטלוג** | ✅ קוד | 100% | PR #13 — טרם מוזג ל-main |
...
*נותר:* למזג PR #13 ל-main כדי שכל סשן עתידי ייהנה מזה. שדה ה-`skill:` במרשם הניכויים מצביע כעת על
סקילים שניתנים למשיכה בפועל.
...
**סקילים (עדיפות #1 הקודמת) — ✅ הוכרע ובוצע:** נבחרה **אופציה #1 — ניקמוט הסקילים לריפו**.
נותר רק **למזג את PR #13 ל-main**. (אם סקיל Tier-2 יתברר כנחוץ תדיר → לקדם אותו ל-Tier 1: לשמור
את תיקייתו תחת `.claude/skills/` ולהוסיף שורה בטבלת ה-core ב-CLAUDE.md.)
